...
#define ERR_SECURE_WINNTLM_NOT_SUPP		PKG_SECURE+12
	errortext(ERR_SECURE_WINNTLM_NOT_SUPP, "Authentication using Windows NTLM security is not supported.")
...
#define	ERR_SECURE_BADMSGDATA			PKG_SECURE+25
	errortext(ERR_SECURE_BADMSGDATA,	"Network error: the message content was unexpected or the message has been corrupted")
...
#define	ERR_SECURE_ID_VAULT_UPLOAD_ACCESS_DENIED			PKG_SECURE+45
	errortext(ERR_SECURE_ID_VAULT_UPLOAD_ACCESS_DENIED,	"Access denied for upload to the ID Vault")
...
#define  STR_PTCONNECT_PT_ESTABLISHED 		PKG_SECURE+227
	errortext(STR_PTCONNECT_PT_ESTABLISHED, "Established passthru session for %A to %A via %s")
...
/* PKG_SECURE3 limit to 0 - 15 
*/

#define ERR_SECURE_PHYSICAL_SERVER_AT_DIFFERENT_SITE		PKG_SECURE3+0
	errortext(ERR_SECURE_PHYSICAL_SERVER_AT_DIFFERENT_SITE, 	"Your current mail server is unavailable. You are being connected to a backup server.")

#define ERR_SECURE_NFL_UNSUPPORTED_FUNC	PKG_SECURE3+1
	errortext(ERR_SECURE_NFL_UNSUPPORTED_FUNC, "Unsupported operation for a Notes ID that is enabled for Notes federated login.")

#define ERR_SECURE_SAML_ASSERTION_REUSE			PKG_SECURE3+2
	errortext(ERR_SECURE_SAML_ASSERTION_REUSE, "This SAML assertion has already been used to authenticate the user to this server, and cannot be used again.")

#define ERR_SECURE_SAML_UNINTENDED_AUDIENCE		PKG_SECURE3+3
	errortext(ERR_SECURE_SAML_UNINTENDED_AUDIENCE, "This server is not the intended audience for this SAML assertion.")

#define ERR_SECURE_PW_AUTH_NOT_ALLOWED			PKG_SECURE3+4
	errortext(ERR_SECURE_PW_AUTH_NOT_ALLOWED, "User's policy does not allow password authentication with the ID vault. Password has not been reset.")

#define ERR_SECURE_PW_NOT_SET					PKG_SECURE3+5
	errortext(ERR_SECURE_PW_NOT_SET, "Your password needs to be reset. Contact your administrator if you need help to reset your password.")

#define ERR_SECURE_SAML_BADTIME				PKG_SECURE3+6
	errortext(ERR_SECURE_SAML_BADTIME, "The SAML assertion failed timestamp validation, which may be caused by clock skew between servers: %1e")

#define STR_SECURE_QVAULT			PKG_SECURE3+7
		stringtext(STR_SECURE_QVAULT,"QVR: Vault |QVR:  |QVR:   No Matches in Vault |QVR:   Revert User |QVR:   Reverted User not found in Vault |QVR:   Reverted User already found in Vault");

		/* Index (1-relative) of substrings within STR_SECURE_QVAULT above */
#define IDX_QVAULT_VAULT				1
#define IDX_QVAULT_ERROR				2
#define IDX_QVAULT_NOMATCHES			3
#define IDX_QVAULT_REVERTUSER			4
#define IDX_QVAULT_REVERTUSERNOTFOUND	5
#define IDX_QVAULT_REVERTUSERALREADY    6

#define STR_SECURE_QVAULT2			PKG_SECURE3+8
	stringtext(STR_SECURE_QVAULT2, "QVR:   Archived User |QVR:  User |QVR:   Skipped user |QVR:   Cleared user |QVR:   Updated user |QVR:   Deleted Archived User ");

#define IDX_QVAULT_ARCHIVEUSER			1
#define IDX_QVAULT_USER					2
#define IDX_QVAULT_SKIPPEDUSER			3
#define IDX_QVAULT_CLEAREDUSER			4
#define IDX_QVAULT_UPDATEDUSER			5
#define IDX_QVAULT_DELETEDARCHIVEDUSER	6

#define STR_SECURE_QVAULT_SUMMARY			PKG_SECURE3+9
	stringtext(STR_SECURE_QVAULT_SUMMARY, "Scan Vault : %1$s, Total %2$d, Updated %3$d, No Update Needed %4$d, Update failures %5$d");
#define ERR_SECURE_BAD_BINARY_KEY			PKG_SECURE3+10
	errortext(ERR_SECURE_BAD_BINARY_KEY, "The version of Notes that you are running does not recognize the key that signed this binary")
