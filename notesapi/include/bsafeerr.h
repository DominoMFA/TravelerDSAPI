...
#define ERR_BSAFE_NSL_UNLOCK_FAILED			(PKG_BSAFE+75)
	errortext (ERR_BSAFE_NSL_UNLOCK_FAILED,		"Notes shared login failed with this ID file")
...
#define ERR_BSAFE_ID_FILE_MERGE_FAIL		(PKG_BSAFE+107)
	errortext(ERR_BSAFE_ID_FILE_MERGE_FAIL,	"ID files cannot be merged.")
...
#define ERR_BSAFE_ICC_UNKNOWN                   (PKG_BSAFE4+4)
	errortext (ERR_BSAFE_ICC_UNKNOWN,           "Unknown error received from cryptographic library")
#define ERR_BSAFE_NSL_UNSUPPORTED_FUNC			(PKG_BSAFE4+5)
	stringtext	(ERR_BSAFE_NSL_UNSUPPORTED_FUNC, "Unsupported operation for a Notes ID that is enabled for Notes shared login.")
#define ERR_BSAFE_ICC_INIT_FAILED				(PKG_BSAFE4+6)
	stringtext	(ERR_BSAFE_ICC_INIT_FAILED, "Cryptographic library could not be found or initialization failed.  Please check your configuration.")
...
#define STR_BSAFE_ID_FILE_MERGE					(PKG_BSAFE_STR+96)
	stringtext (STR_BSAFE_ID_FILE_MERGE,		"Name change in progress")
#define IDX_STR_BSAFE_IDMRG_NC_INPROG	1
#define	ERR_BSAFE_CREATECERTIFICATE_ANDWAIT		(PKG_BSAFE_STR+97)
	internaltext(ERR_BSAFE_CREATECERTIFICATE_ANDWAIT,"Prompt for certificate in UI and wait for response")		

#define STR_BSAFE_ID_FILE_MERGE_FAIL_STR		(PKG_BSAFE_STR+98)
	stringtext(STR_BSAFE_ID_FILE_MERGE_FAIL_STR,	"ID files cannot be merged: %s.")

#define STR_BSAFE_SEC_TOK_FMT					(PKG_BSAFE_STR+99)
	stringtext(STR_BSAFE_SEC_TOK_FMT,			"Security token format received is %s")
#define STR_BSAFE_USER_AUTHN_BY_SVC				(PKG_BSAFE_STR+100)
	stringtext(STR_BSAFE_USER_AUTHN_BY_SVC,		"User %1$s authenticated by Kerberos service %2$s")
#define STR_BSAFE_SSPI_AUTHN_FAIL				(PKG_BSAFE_STR+101)
	stringtext(STR_BSAFE_SSPI_AUTHN_FAIL,		"SSPI authentication failed - native security status code 0x%X")
#define STR_BSAFE_KERB_TO_LTPA_NAME				(PKG_BSAFE_STR+102)
	stringtext(STR_BSAFE_KERB_TO_LTPA_NAME,		"Kerberos name %1$s mapped to LTPA name %2$s")
#define STR_BSAFE_SEC_TOK_FMTS					(PKG_BSAFE_STR+103)
	stringtext(STR_BSAFE_SEC_TOK_FMTS,			"Unknown|SPNEGO NegTokenInit|SPNEGO NegTokenTarg|NTLM|Kerberos")
#define IDX_STR_BSAFE_TKFMT_UNK		1
#define IDX_STR_BSAFE_TKFMT_SPN_NTI	2
#define IDX_STR_BSAFE_TKFMT_SPN_NTT	3
#define IDX_STR_BSAFE_TKFMT_NTLM	4
#define IDX_STR_BSAFE_TKFMT_KERB	5

#define STR_BSAFE_ERR_LOAD_SEC_LIB				(PKG_BSAFE_STR+104)
	stringtext(STR_BSAFE_ERR_LOAD_SEC_LIB,		"Error loading security library %s")
#define STR_BSAFE_ERR_ACQ_CREDS					(PKG_BSAFE_STR+105)
	stringtext(STR_BSAFE_ERR_ACQ_CREDS,			"Error acquiring the credentials handle (%s) - native security status code 0x%X")
#define STR_BSAFE_ERR_MEM_SPNEGO				(PKG_BSAFE_STR+106)
	stringtext(STR_BSAFE_ERR_MEM_SPNEGO,		"Memory error in SPNEGO negotiation")
#define STR_BSAFE_ERR_TOKEN_CONT				(PKG_BSAFE_STR+107)
	stringtext(STR_BSAFE_ERR_TOKEN_CONT,		"Error retrieving token on SPNEGO AcceptToken continuation: %e")
#define STR_BSAFE_ERR_SPNEGO_NAME				(PKG_BSAFE_STR+108)
	stringtext(STR_BSAFE_ERR_SPNEGO_NAME,		"Error retrieving SPNEGO name: %e")
#define STR_BSAFE_ERR_NULL_SPNEGO_NAME			(PKG_BSAFE_STR+109)
	stringtext(STR_BSAFE_ERR_NULL_SPNEGO_NAME,	"Null SPNEGO name")
#define STR_BSAFE_AUTHN_USER_VIA				(PKG_BSAFE_STR+110)
	stringtext(STR_BSAFE_AUTHN_USER_VIA,		"Authenticated user is %1$s via %2$s")
#define STR_BSAFE_SSPI_ATTR_MISMATCH			(PKG_BSAFE_STR+111)
	stringtext(STR_BSAFE_SSPI_ATTR_MISMATCH,	"SSPI security attributes received 0x%X, but requested 0x%X")
#define STR_BSAFE_SSPI_UNSUPP_RET_CODE			(PKG_BSAFE_STR+112)
	stringtext(STR_BSAFE_SSPI_UNSUPP_RET_CODE,	"Unsupported SSPI return code %s")
#define STR_BSAFE_HTTP_CLIENT_NTLM				(PKG_BSAFE_STR+113)
	stringtext(STR_BSAFE_HTTP_CLIENT_NTLM,		"Attempt by HTTP client %s to authenticate using Windows NTLM security is not supported")
#define STR_BSAFE_START_SPNEGO_NEG				(PKG_BSAFE_STR+114)
	stringtext(STR_BSAFE_START_SPNEGO_NEG,		"Starting SPNEGO Negotiate - a properly configured HTTP client should send an Authorization: Negotiate header containing SPNEGO token when repeating the request %s")
#define STR_BSAFE_ERR_ACCEPT_CREDS				(PKG_BSAFE_STR+115)
	stringtext(STR_BSAFE_ERR_ACCEPT_CREDS,		"Error accepting the security context (%s) - native security status code 0x%X")
#define STR_BSAFE_ERR_ACCEPT_CONT				(PKG_BSAFE_STR+116)
	stringtext(STR_BSAFE_ERR_ACCEPT_CONT,		"Accepting the security context (%s) requires continuation")
#define STR_BSAFE_ERR_QUERY_ATTRS				(PKG_BSAFE_STR+117)
	stringtext(STR_BSAFE_ERR_QUERY_ATTRS,		"Error querying attributes (%s) - native security status code 0x%X")
#define STR_ASN1_ILL_OID						(PKG_BSAFE_STR+118)
	stringtext (STR_ASN1_ILL_OID,				"Unexpected ASN.1 Object ID field %s")
#define STR_BSAFE_ERR_ROUTINE_PASS				(PKG_BSAFE_STR+119)
	stringtext (STR_BSAFE_ERR_ROUTINE_PASS,		"Success calling native routine %s")
#define STR_BSAFE_ERR_WINDOWS_ROUTINES			(PKG_BSAFE_STR+120)
	stringtext (STR_BSAFE_ERR_WINDOWS_ROUTINES,	"AcquireCredentialsHandleW|AcceptSecurityContext|QueryContextAttributesW|QuerySecurityPackageInfoW")
#define IDX_STR_BSAFE_ERR_WINDOWS_ROUTINES_ACQUIRE		1
#define IDX_STR_BSAFE_ERR_WINDOWS_ROUTINES_ACCEPT		2
#define IDX_STR_BSAFE_ERR_WINDOWS_ROUTINES_QUERYCTX		3
#define IDX_STR_BSAFE_ERR_WINDOWS_ROUTINES_QUERYPKG		4
