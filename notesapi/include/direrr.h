...
/*                                                                   */
/* Copyright IBM Corp. 2007, 2019  All Rights Reserved.              */
...
#define ERR_DIR_LLNDIRSYNC_NOCONNECTION			PKG_DIR+31
	errortext(ERR_DIR_LLNDIRSYNC_NOCONNECTION,	"LLNDirSync requires a Connection doc whose Filenames field contains the customer NAB's hosted pathname.");
#define ERR_DIR_LLNDIRSYNC_NODOMAIN				PKG_DIR+32
	errortext(ERR_DIR_LLNDIRSYNC_NODOMAIN,		"LLNDirSync requires that the customer NAB have a Domain in its Directory Profile.");
#define ERR_DIR_TDS_NO_CHANGELOG_OBJECT			PKG_DIR+33
	errortext(ERR_DIR_TDS_NO_CHANGELOG_OBJECT,	"TDS Change Log is not enabled for LLNDirSync!  Follow 'LotusLive Notes 2 Operations Guide' for enablement instructions.");
#define ERR_DIR_INVALID_LLN2_LDAPDN				PKG_DIR+34
	errortext(ERR_DIR_INVALID_LLN2_LDAPDN,		"The TDS LDAP DN is missing the expected DC=<domain>, CN=<customerid> name components.");
#define ERR_DIR_TDS_EXPECTED_HITS				PKG_DIR+35
	errortext(ERR_DIR_TDS_EXPECTED_HITS,		"TDS search for %s incorrectly returned no results when one was expected.  See 'Correcting directory servers that incorrectly return no search results' in 'LotusLive Notes 2 Operations Guide' for resolution.")
...
/* additional PKG_DIR2 codes will go here */
#endif
