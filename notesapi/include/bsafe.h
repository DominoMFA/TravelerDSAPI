...
STATUS LNPUBLIC SECReattachToRootIDExt (BOOL bCloseSessions); /* close Sessions for calling Thread Group */
...
STATUS LNPUBLIC SECCreateTokenListEntry( SECTOKENFORMAT TokenType, 
								 ...
								 BOOL bSecureOnly, /* if set, will only send this token over a secure channel */
...
/* SSO cookie names config info*/
#define MAXLTPACOOKIENAMELEN 		128

typedef struct
{
	MEMHANDLE	mhItem;
	char*		ptr;
} LTPA_COOKIENAME_ITEM;

typedef enum
{
	LTPATOKENCOOKIENAME = 0,
	LTPATOKEN2COOKIENAME,
	MAXLTPATOKENCOOKIENAMES
} LTPACOOKIENAMES;
...
typedef struct
{
	MEMHANDLE					mhSelf;
	DWORD						dwSize;
	LTPA_COOKIENAME_ITEM		ltpaCookieNames[MAXLTPATOKENCOOKIENAMES];
} LTPA_COOKIENAMES;

STATUS LNPUBLIC SECGetSSOCookieNames (
	char					*ServerName,	/* Reserved as NULL (ignored) */
	char					*OrgName,
	char					*ConfigName,
	LTPA_COOKIENAMES		**cookieNames,
	DWORD					dwReserved,
	void					*vpReserved
	);

void LNPUBLIC SECFreeSSOCookieNames (
	LTPA_COOKIENAMES		**cookieNames,
	DWORD					dwReserved,
	void					*vpReserved
	);
