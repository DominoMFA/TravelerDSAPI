...
/* Reclaimed in 8.51 */
#define ERR_DA_DOMAIN_NOTFOUND		PKG_NIF+44
	errortext(ERR_DA_DOMAIN_NOTFOUND,	"Domain <%s> was not found in this server's directory domain tables.  Verify this domain with your DA Configuration")
/* */
...
/*	NOTE: NIF2 errors from PKG_NIF2+0 - PKG_NIF2+87 */
...
#define ERR_NAMELOOKUP_DOMAIN_UNAVAILABLE  	PKG_NIF2+15
	errortext(ERR_NAMELOOKUP_DOMAIN_UNAVAILABLE,  "One or more Directory Assistance Domains used to process directory/NAMELookup requests was unavailable.")
...
#define ERR_LOOKUP_DBNAME_NOT_FOUND_WITHOUT_PARAMS			PKG_NIF2+47
	errortext(ERR_LOOKUP_DBNAME_NOT_FOUND_WITHOUT_PARAMS,	"WARNING: NAMELookups from this server to the remote Domino Directory, when Extended ACLs are enabled is prohibited.  Correct by creating a Directory Assistance document for it.")
...
#define LOG_NIF_DIRECTORY_SERVING						PKG_NIF2+65
	errortext(LOG_NIF_DIRECTORY_SERVING,				"Directory %p for '%s' in use")
...
#define ERR_REBUILD_MISSING_ATOM		 	PKG_NIF2+72
	errortext(ERR_REBUILD_MISSING_ATOM,			"Informational, rebuild view needed - missing atom table (reading %s %s note Title:'%s')")
...
#define ERR_GBCACHE_CONTAINER_NOT_VALID		PKG_NIF2+75
	errortext(ERR_GBCACHE_CONTAINER_NOT_VALID, 	"gbiCache container failed consistency check")
...
#define NIF_VIEW_REG_CORRUPT				PKG_NIF2+82
	errortext(NIF_VIEW_REG_CORRUPT,			"View Registration Corrupt")
#define NIF_VIEW_REG_OVERFLOW				PKG_NIF2+83
	errortext(NIF_VIEW_REG_OVERFLOW,			"Exceeded maximum number of updates (%d) for db: %s, view: %s")

#define ERR_NIF_LDAP_UPDATE					PKG_NIF2+84
	errortext(ERR_NIF_LDAP_UPDATE,			"Failed update on LDAP Server %s, DN: %s, Reason: %s")

#define STR_NIF_LDAP_TELEPHONE_COLUMN		PKG_NIF2+85
	stringtext(STR_NIF_LDAP_TELEPHONE_COLUMN,		"(office)  |(home)  ")
#define STR_NIF_LDAP_TELEPHONE_OFFICE_IDX	1
#define STR_NIF_LDAP_TELEPHONE_HOME_IDX		2

#define ERR_NIF_DA_CRED_ENCRYPT			PKG_NIF2+86
	errortext(ERR_NIF_DA_CRED_ENCRYPT,	"Skipping encrypting DA configuration for %s")

#define ERR_NIF_INVCALLBACKFCT                           PKG_NIF2+87
	errortext(ERR_NIF_INVCALLBACKFCT,               "Illegal callback function")

/* 	NOTE: NIF2 errors can only go up to PKG_NIF2+87.  The rest of the codes were stolen for PKG_DIR */
