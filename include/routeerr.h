...
#define ERR_ROUTER_MSGQ_POOLFULL 	(PKG_ROUTER+17)
	errortext(ERR_ROUTER_MSGQ_POOLFULL, "Insufficient Queue Space - Router message queue is full.")
#define ERR_ROUTER_TRANSFER_Q_POOLFULL 	(PKG_ROUTER+18)
	errortext(ERR_ROUTER_TRANSFER_Q_POOLFULL, "Insufficient Queue Space - Router transfer queue is full.")
...
#define ERR_ROUTERPOOL_FULL 						(PKG_ROUTER+78)
	errortext(ERR_ROUTERPOOL_FULL,					"Insufficient memory - Router pool is full.")
...
#define ERR_ROUTER_OOO_REPORT_ERROR					(PKG_ROUTER2+16)
	errortext(ERR_ROUTER_OOO_REPORT_ERROR,			"Router: Error generating out of office report in '%s'")
...
#define ERR_ROUTER_OOO_DAILYREPORT_ERROR			(PKG_ROUTER2+21)
	errortext(ERR_ROUTER_OOO_DAILYREPORT_ERROR,		"Router: Error during daily maintenance check for out of office service in '%p': %e")
...
#define ERR_MBOX_NOT_AVAILABLE						(PKG_ROUTER2+31)
	errortext(ERR_MBOX_NOT_AVAILABLE,				"Router: Mailbox is not available")
#define ERR_MBOX_DBGS_NOT_ENABLED					(PKG_ROUTER2+32)
	errortext(ERR_MBOX_DBGS_NOT_ENABLED,			"Router: Can't request a specific mailbox because DBGroups is not enabled.")
#define ERR_OOO_MISSING_PARAM						(PKG_ROUTER2+33)
	errortext(ERR_OOO_MISSING_PARAM,					"OOOAPI: Manditory parameter is not set")
#define ERR_OOO_NOACCESS							(PKG_ROUTER2+34)
	errortext(ERR_OOO_NOACCESS,						"OOOAPI: Insufficient access to turn on Out of office.  Minimum of Editor level access is required.")
#define ERR_OOO_MAILINFO_FAIL						(PKG_ROUTER2+35)
	errortext(ERR_OOO_MAILINFO_FAIL,				"OOOAPI: Failed to obtain information about home mail server")
#define ERR_OOO_DUPLICATE_NAME						(PKG_ROUTER2+36)
	errortext(ERR_OOO_DUPLICATE_NAME,				"OOOAPI: Duplicate name found in Domino Directory ")
#define ERR_OOO_MISSING_MAILFILE					(PKG_ROUTER2+37)
	errortext(ERR_OOO_MISSING_MAILFILE,				"OOOAPI: Unable to open user's mail file")
#define ERR_ROUTER_OOO_PROFILE_ERROR				(PKG_ROUTER2+38)
	errortext(ERR_ROUTER_OOO_PROFILE_ERROR,			"OOOAPI: Unable to open out of office profile in database %p: %e")
#define ERR_OOO_INVALID_TIMEPARM					(PKG_ROUTER2+39)
	errortext(ERR_OOO_INVALID_TIMEPARM,				"OOOAPI: Invalid time parameter")
#define ERR_OOO_MISSING_INIT						(PKG_ROUTER2+40)
	errortext(ERR_OOO_MISSING_INIT,					"OOOAPI: Invalid Initialization. Most likely OOOInit was not called")
#define ERR_OOO_MSGTOOLARGE							(PKG_ROUTER2+41)
	errortext(ERR_OOO_MSGTOOLARGE,					"OOOAPI: Body of the message is too large")
#define ERR_OOO_NAMELOOKUP							(PKG_ROUTER2+42)
	errortext(ERR_OOO_NAMELOOKUP,					"OOOAPI: Namelookup for user '%s' on server '%s' failed")
#define ERR_OOO_NAMELOOKUPATTR						(PKG_ROUTER2+43)
	errortext(ERR_OOO_NAMELOOKUPATTR,				"OOOAPI: Extended directory '%a' on server '%a' does not have attributes 'MailServer' and 'MailFile'. This information is required for enabling Out of Office")
#define ERR_ROUTER2_SCHEDULED_MAIL_RESTRICTION		(PKG_ROUTER2+44)
	errortext(ERR_ROUTER2_SCHEDULED_MAIL_RESTRICTION, "This scheduled message cannot be delivered because of a server limit. You can create it again and send it unscheduled.")
...
#define STR_ROUTER_OOO_REPORT_START					(PKG_ROUTER_STR+8)
	stringtext(STR_ROUTER_OOO_REPORT_START,			"Router: Beginning Out of office service daily maintenance.")
#define STR_ROUTER_OOO_REPORT_END					(PKG_ROUTER_STR+9)
	stringtext(STR_ROUTER_OOO_REPORT_END,			"Router: Completed Out of office service daily maintenance.")
...
#define STR_ROUTER_OOO_RETURNING					(PKG_ROUTER_STR+15)
	stringtext(STR_ROUTER_OOO_RETURNING,			"(returning %z)")
#define STR_ROUTER_OOO_AWAY_BODY					(PKG_ROUTER_STR+16)
	stringtext(STR_ROUTER_OOO_AWAY_BODY,			"I will be out of the office starting %z and will not return until %z")
#define STR_ROUTER_OOO_SUBJ_DEFAULT					(PKG_ROUTER_STR+17)
	stringtext(STR_ROUTER_OOO_SUBJ_DEFAULT,			"%s is out of the office")
#define STR_ROUTER_OOO_DONOTDELETE					(PKG_ROUTER_STR+18)
	stringtext(STR_ROUTER_OOO_DONOTDELETE,			"Please do not delete - Out of office Profile (created by OOO API)")
#define STR_ROUTER_OOO_CALENTRY_SUBJECT				(PKG_ROUTER_STR+19)
	stringtext(STR_ROUTER_OOO_CALENTRY_SUBJECT,		"Out Of Office")
